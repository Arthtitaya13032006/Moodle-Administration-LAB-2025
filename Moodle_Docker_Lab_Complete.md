...
**บันทึกผลการทดลอง 6:**
<img width="1902" height="975" alt="image" src="https://github.com/user-attachments/assets/3bf901b3-2999-409f-adf6-c46157abac30" />
...
**บันทึกผลการทดลอง 7:**
```
<img width="1902" height="971" alt="image" src="https://github.com/user-attachments/assets/063c2b1e-0747-4a40-b683-8dd60950652c" />
...
**บันทึกผลการทดลอง 8:**
```
<img width="1903" height="972" alt="image" src="https://github.com/user-attachments/assets/d211258e-f992-4520-bf49-2695320816ea" />
...
**บันทึกผลการทดลอง 9:**
```
<img width="1894" height="972" alt="image" src="https://github.com/user-attachments/assets/8e2f66c2-9b1d-4187-a3a7-0a5f879a4308" />
...
**บันทึกผลการทดลอง 10:**
<img width="1918" height="971" alt="image" src="https://github.com/user-attachments/assets/e0a57fa7-0e47-4477-b5fd-f6eb598dd541" />
...
5. 
   **บันทึกผลการทดลอง 11:**
```
<img width="1919" height="966" alt="image" src="https://github.com/user-attachments/assets/28b4ea83-acd2-483f-88ba-d8024c4297c2" />
...
คำตอบ:
```
Docker Image คือแม่แบบ (template) ที่บรรจุระบบปฏิบัติการ ไลบรารี และแอปพลิเคชัน
...
คำตอบ:
```
Network ทำหน้าที่เป็น เครือข่ายกลาง ให้ Container ติดต่อกันได้
...
คำตอบ:
```
depends_on ใช้กำหนดลำดับการเริ่มต้น Container
...
คำตอบ: 
```
แก้ไขที่ส่วน ports ของ service moodle
ports:
  - "9000:80"
```
...
คำตอบ:
```
db คือชื่อ service ของ Container ฐานข้อมูล Docker DNS จะ resolve ชื่อ db ไปยัง IP ของ Container ฐานข้อมูล
...
วิธีตรวจสอบ:1.ตรวจสอบ log
          ...
          3.ตรวจสอบค่า Environment Variables
            ...
            MOODLE_DB_USER
            MOODLE_DB_PASSWORD
          4. ตรวจสอบ Network ของ Docker
          5.Restart Container
          6. รีสตาร์ท Container เพื่อทดสอบอีกครั้ง
วิธีแก้ไข: วิธีแก้ไข:
รันคำสั่ง docker compose ps หากพบว่า DB ไม่ทำงาน ให้ใช้ docker compose up -d หรือ docker compose restart db
วิธีแก้ไข:
ใช้คำสั่ง docker compose logs moodle และ docker compose logs db เพื่อตรวจสอบข้อความผิดพลาด
วิธีแก้ไข:
แก้ไขค่าให้ถูกต้อง แล้วรัน docker compose down และ docker compose up -d ใหม่
วิธีแก้ไข:
ตรวจสอบว่า moodle_app และ moodle_db อยู่ใน Network เดียวกันใน docker-compose.yml
วิธีแก้ไข:
ใช้คำสั่ง docker compose restart db และ docker compose restart moodle
...
หยุดและลบ Container
ลบ Volumes ทั้งหมด
ผลลัพธ์ ข้อมูลฐานข้อมูล
ข้อมูล Moodle ทั้งหมด
หายถาวร ไม่สามารถกู้คืนได้
